...
if (isset($_GET['ajax_action'])) {
    ...
    if ($_GET['ajax_action'] == 'load_details') {
        ...
        // Project info
        $res = $conn->query("SELECT p.*, c.name as client_name, d.name as dept_name FROM projects p JOIN clients c ON p.client_id = c.id LEFT JOIN departments d ON p.department_id = d.id WHERE p.id = $id AND p.client_id = $client_id");
        ...
        while ($row = $res->fetch_assoc()) {
            $ms_id = $row['id'];
            $subs = [];
            $sub_res = $conn->query("SELECT sm.*, a.name as assignee_name, d.name as dept_name FROM project_sub_milestones sm LEFT JOIN admins a ON sm.assigned_to_admin = a.id LEFT JOIN departments d ON sm.assigned_to_department = d.id WHERE sm.milestone_id = $ms_id ORDER BY sm.created_at ASC");
    ...
    if ($_GET['ajax_action'] == 'get_milestone_reports') {
        ...
        $check = $conn->query("SELECT p.client_id FROM project_milestones m JOIN projects p ON m.project_id = p.id WHERE m.id = $milestone_id");
    ...
    if ($_GET['ajax_action'] == 'add_milestone_report') {
        ...
        $check = $conn->query("SELECT p.id as project_id, p.client_id FROM project_milestones m JOIN projects p ON m.project_id = p.id WHERE m.id = $milestone_id");
        $ms = $check->fetch_assoc();
        if (!$ms || $ms['client_id'] != $client_id) {
...
<head>
    ...
    <title>WILSOVLEWEL | Client Projects</title>
...
<body class="bg-surface font-body text-on-surface selection:bg-primary/20 selection:text-primary site-gradient-bg flex min-h-screen">
    ...
    <div class="flex-1 flex flex-col min-w-0 mt-16 h-[calc(100vh-4rem)] overflow-hidden relative z-10">
        <!-- Header -->
        <header class="h-16 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-6 shrink-0 z-20">
            <div>
                <h1 class="text-lg font-bold font-headline text-slate-900 leading-tight">System Ledger</h1>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Project Tracking</p>
            </div>
            <a href="propose_project.php" class="bg-primary text-on-primary px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2 hover:shadow-lg transition-all active:scale-95">
                <span class="material-symbols-outlined text-sm">add_circle</span> NEW PROPOSAL
            </a>
        </header>
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <div class="flex-1 bg-slate-50 hidden lg:flex flex-col relative" id="detailCanvas">
                <div class="absolute inset-0 flex items-center justify-center text-center p-8 z-0">
                    <div>
                        <span class="material-symbols-outlined text-6xl text-slate-200 mb-4 block">space_dashboard</span>
    ...
    <!-- Mobile Offcanvas Wrapper for Details -->
    ...
    <div id="mobileDetailDrawer" class="fixed inset-y-0 right-0 w-full max-w-md bg-slate-50 z-[110] lg:hidden transform translate-x-full transition-transform duration-300 shadow-2xl flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar flex flex-col relative" id="mobileDetailContent">
            <!-- Content cloned from detailCanvas here -->
        </div>
    </div>
    ...
    <div id="msChatModal" class="modal-overlay">
        <div class="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
                <div>
                    <h3 class="font-bold font-headline text-slate-900" id="msChatTitle">Milestone Discussion</h3>
        ...
        reports.forEach(r => {
            ...
            html += `
                <div class="flex flex-col ${isMe ? 'items-end' : 'items-start'} mb-4">
                    ${!isMe ? `<span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1 ml-1">${r.sender_name}</span>` : ''}
                    <div class="${isMe ? 'bg-primary text-on-primary rounded-tr-none' : 'bg-white border border-slate-100 rounded-tl-none'} rounded-2xl px-4 py-2.5 max-w-[85%] shadow-sm">
                        <p class="text-xs font-medium leading-relaxed whitespace-pre-wrap">${r.content}</p>
                        <p class="text-[9px] font-bold opacity-60 mt-1 uppercase tracking-widest">${new Date(r.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                    </div>
                </div>
            `;
        });
    ...
    document.getElementById('msChatForm').addEventListener('submit', async function(e) {
        ...
        container.innerHTML += `
            <div class="flex flex-col items-end mb-4 opacity-50">
                <div class="bg-primary text-on-primary rounded-2xl rounded-tr-none px-4 py-2.5 max-w-[85%] shadow-sm">
                    <p class="text-xs font-medium leading-relaxed whitespace-pre-wrap">${content}</p>
                    <p class="text-[9px] font-bold opacity-60 mt-1 uppercase tracking-widest">Sending...</p>
                </div>
            </div>
        `;
        container.scrollTop = container.scrollHeight;
